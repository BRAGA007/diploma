...
DATABASE_URI = str(settings.database_uri).replace("+asyncpg", "").replace("%", "%%")
...
def run_migrations_online() -> None:
    ...
    ) -> None:  # Не создавать пустые миграции
        if hasattr(config, "cmd_opts") and getattr(config.cmd_opts, "autogenerate", False):
            script = directives[0]
